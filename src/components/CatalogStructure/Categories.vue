    ...
    <div class="categories main_block_style">
        <div class="categories_inner">
            <div v-for="(cat, index) in allCategories" :key="index" class="category_item">
                ...
                <router-link v-if="cat.is_endpoint" :to="{ name: 'Catalog', params: { slug: cat.slug }}">{{cat.title}}</router-link>
                <router-link v-else :to="{ path: '/catalog/categories', query: { parent: cat.slug }}">{{cat.title}}</router-link>
            </div>
        </div>
    </div>
